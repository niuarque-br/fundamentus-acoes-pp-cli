# Fundamentus Acoes Printed CLI Agent Guide

This directory is a generated `fundamentus-acoes-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
fundamentus-acoes-pp-cli doctor --json
fundamentus-acoes-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
fundamentus-acoes-pp-cli which "<capability>" --json
fundamentus-acoes-pp-cli <command> --help
```
...
```bash
fundamentus-acoes-pp-cli <command> --agent
```
...
```bash
fundamentus-acoes-pp-cli <command> --help
fundamentus-acoes-pp-cli <command> --dry-run --agent
```
