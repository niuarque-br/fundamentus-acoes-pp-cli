...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "fundamentus-acoes-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FUNDAMENTUS_ACOES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FUNDAMENTUS_ACOES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fundamentus-acoes-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/fundamentus-acoes-pp-cli/feedback.jsonl.
When ` + "`FUNDAMENTUS_ACOES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FUNDAMENTUS_ACOES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fundamentus-acoes-pp-cli feedback list
  fundamentus-acoes-pp-cli feedback list --limit 5
  fundamentus-acoes-pp-cli feedback list --json`,
